...
Distributor.prototype={
	...
	compressText:function(src){
		var r=UTF8BOM(fs.readFileSync(src)),e=path.extname(src);
		if(e=='.html') [
			[/<!--.*?-->/g,''],
			[/<\s+/g,'<'],
			[/\s+>/g,'>'],
			[/>( |)\s*</g,'>$1<'],	// keep one space
			[/(^|\s)\s*/g,'$1'],
		].forEach(function(i){r=r.replace(i[0],i[1]);});
		else if(e=='.json')
			r=JSON.stringify(JSON.parse(r));
		return r;
	},
	packTextFile:function(src,dest){
		...
		t.getStream(dest,function(f){
			if(/\.js$/.test(src)) {
				r=uglifyjs.minify(src);
				f.write(r.code,'utf-8',finish);
			} else if(/\.css$/.test(src)) {
				less.render(UTF8BOM(fs.readFileSync(src)),{
					compress:true,
				},function(e,r){
...
function main(){
	...
	console.log('Packager for Maxthon addons - written in NodeJS by Gerald')
	d=JSON.parse(UTF8BOM(fs.readFileSync(path.join(src,'def.json'))))[0];
