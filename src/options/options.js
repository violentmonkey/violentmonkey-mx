...
var Transporter = function() {
		...
		function getFile() {
			...
			if(entry) {
				if(userjs.test(entry.filename)) entry.getData(writer, function(text) {
					...
					post({cmd: 'ParseScript', data: script}, function() {
						count ++;
						getFile();
					});
				}); else getFile();
	...
	function bindEvents() {
		...
		btExport.addEventListener('click', function(e) {
			...
			scriptList.forEach(function(data, i) {
				if(xList.childNodes[i].classList.contains('selected'))
					ids.push(data.script.id);
			});
			post({
				cmd: 'ExportZip',
				data: {
		...
		xList.addEventListener('click', function(e) {
			var target = e.target;
			if(target.parentNode === this) target.classList.toggle('selected');
		});
		$('#bSelect').addEventListener('click', function(e) {
			...
			var cmd = state ? 'add' : 'remove';
			Array.prototype.forEach.call(nodes, function(node) {
				node.classList[cmd]('selected');
			});
		}, false);
	}
	...
	return {
		initSettings: function(settings) {
			cbValues.checked = settings.exportValues;
		},
		initList: function() {
			xList.innerHTML = '';
...
!function() {
	...
	$('#bNew').addEventListener('click', function(e) {
		post({cmd:'NewScript'}, Editor.editScript);
	}, false);
	...
	var updateSuboptions = function(e) {
		Array.prototype.forEach.call(
			e.parentNode.nextElementSibling.querySelectorAll('input'),
			function(input){input.disabled = !e.checked;}
		);
	};
